...
# ── Defaults ──────────────────────────────────────────────────────────────────
ADCLAW_HOME="${ADCLAW_HOME:-$HOME/.adclaw}"
...
printf "${GREEN}[adclaw]${RESET} Installing AdClaw into ${BOLD}%s${RESET}\n" "$ADCLAW_HOME"
...
if [ -d "$ADCLAW_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$ADCLAW_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$ADCLAW_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$ADCLAW_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/adclaw/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/adclaw/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/adclaw/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing AdClaw from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$ADCLAW_VENV/bin/python" --prerelease=allow
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing AdClaw from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$ADCLAW_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$ADCLAW_VENV/bin/python" --prerelease=allow
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console
    fi
else
    PACKAGE="adclaw"
    if [ -n "$VERSION" ]; then
        PACKAGE="adclaw==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$ADCLAW_VENV/bin/python" --prerelease=allow --quiet
fi

# Verify the CLI entry point exists
[ -x "$ADCLAW_VENV/bin/adclaw" ] || die "Installation failed: adclaw CLI not found in venv"
info "AdClaw installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$ADCLAW_VENV/bin/python" -c "import importlib.resources, adclaw; p=importlib.resources.files('adclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$ADCLAW_BIN/adclaw" << 'WRAPPER'
#!/usr/bin/env bash
# AdClaw CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

ADCLAW_HOME="${ADCLAW_HOME:-$HOME/.adclaw}"
REAL_BIN="$ADCLAW_HOME/venv/bin/adclaw"
...
chmod +x "$ADCLAW_BIN/adclaw"
info "Wrapper created at $ADCLAW_BIN/adclaw"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.adclaw/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.adclaw/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}AdClaw installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$ADCLAW_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$ADCLAW_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}adclaw init${RESET}       # first-time setup\n"
printf "  ${BOLD}adclaw app${RESET}        # start AdClaw\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}adclaw uninstall${RESET}\n"
