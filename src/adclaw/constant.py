...
WORKING_DIR = (
    Path(os.environ.get("ADCLAW_WORKING_DIR", "~/.adclaw"))
    .expanduser()
    .resolve()
)
SECRET_DIR = (
...
JOBS_FILE = os.environ.get("ADCLAW_JOBS_FILE", "jobs.json")

CHATS_FILE = os.environ.get("ADCLAW_CHATS_FILE", "chats.json")

CONFIG_FILE = os.environ.get("ADCLAW_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = os.environ.get("ADCLAW_HEARTBEAT_FILE", "HEARTBEAT.md")
...
RUNNING_IN_CONTAINER = os.environ.get("ADCLAW_RUNNING_IN_CONTAINER", "false")
...
try:
    MODEL_PROVIDER_CHECK_TIMEOUT = float(
        os.environ.get("ADCLAW_MODEL_PROVIDER_CHECK_TIMEOUT", "5.0"),
    )
...
DOCS_ENABLED = os.environ.get("ADCLAW_OPENAPI_DOCS", "false").lower() in (
    "true",
    "1",
    "yes",
)
...
# Custom channel modules (installed via `adclaw channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = int(
    os.environ.get("ADCLAW_MEMORY_COMPACT_KEEP_RECENT", "3"),
)

MEMORY_COMPACT_RATIO = float(
    os.environ.get("ADCLAW_MEMORY_COMPACT_RATIO", "0.7"),
)
...
# Example: ADCLAW_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = os.environ.get("ADCLAW_CORS_ORIGINS", "").strip()
